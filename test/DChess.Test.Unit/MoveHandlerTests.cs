...
public class MoveHandlerTests : MoveHandlerTestBase
{
    ...
    {
        Sut.Set(fenString);
        MoveHandler.GetGameStateScore(Sut, White).Should().Be(expectedScore, $"Lichess url: {Sut.AsLichessUrl}", Sut,
            "Game Score Visualization");
    }
    ...
    {
        move.Format().Should().Be(expected.Format(), "the moves should match", Sut, "Move Comparison Visualization");
    }

    private void ShouldNotBe(Move move, Move expectedNotToBe, string because)
    {
        move.Format().Should().NotBe(expectedNotToBe.Format(), because, Sut, "Move Comparison Visualization");
    }
    ...
    public void bishop_does_not_capture_pawn_when_it_will_be_recaptured()
    {
        ...
        bestMove.Should().NotBe(losingMove,
            "the bishop will be immediately recaptured by the king, resulting in a net loss", Sut,
            "Bishop Capture Visualization");
    }
    ...
    public void game_state_score_correctly_evaluates_material_advantage(string fenString, int expectedScore)
    {
        ...
        MoveHandler.GetGameStateScore(Sut, White).Should().Be(expectedScore,
            "the score should reflect material advantage", Sut, "Material Advantage Visualization");
    }

    [Theory(DisplayName = "Game state score correctly evaluates check and checkmate")]
    [InlineData(NoCheckNoMaterialAdvantage, 0)]
    [InlineData(BlackKingInCheckWith1PawnMaterialAdvantage,
        Weights.Material.Pawn + Weights.GameState.Checkmate)] // Checkmate with pawn
    [InlineData(BlackCheckMateWith2PawnWhiteAdvantage,
        Weights.Material.Pawn * 2 + Weights.GameState.Checkmate)] // Checkmate with 2 pawns
    public void game_state_score_correctly_evaluates_check_and_checkmate(string fenString, int expectedScore)
    {
        Sut.Set(fenString);
        MoveHandler.GetGameStateScore(Sut, White).Should().Be(expectedScore,
            "the score should reflect check/checkmate status", Sut, "Check/Checkmate Visualization");
    }
}
